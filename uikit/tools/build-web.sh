...
emsdk_dir="$nativekit_dir/.tools/emsdk"
...
if [[ ! -f "$emsdk_dir/emsdk_env.sh" ]]; then
    echo "Emscripten is not installed. Run $nativekit_dir/tools/setup-web.sh first." >&2
    exit 1
fi
...
emcmake cmake -S "$repo_dir" -B "$build_dir" -G Ninja \
    ...
    -DNKUI_NATIVEKIT_DIR="$nativekit_dir" \
...
echo "  $build_dir/nativekit/tests/nativekit_platform_parity.html"
echo "  $build_dir/nativekit/tests/nativekit_web_accessibility.html"
echo "  $build_dir/nativekit/tests/nativekit_web_system_equivalents.html"
echo
