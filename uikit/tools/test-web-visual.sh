...
artifact_dir=${NATIVEKIT_WEB_ARTIFACT_DIR:-"$build_dir"}
...
haxeon_dir=${haxeon_dir:-"$repo_dir/../haxeon"}
haxe_source_dirs=(
    "$repo_dir/haxe"
    "$repo_dir/bindings"
...
for guest_input in "$repo_dir/tools/showcase-wasm.sh" \
    "$repo_dir/cmake/wasm_memory_contract.json.in"; do
...
for visual_case in "${cases[@]}"; do
    ...
    arguments=(--debug-port "$debug_port" --page-url "$page_url" --width "$width" --height "$height"
        --scale "$device_scale"
        --reference "$repo_dir/tests/golden/showcase-${case_name}.png"
