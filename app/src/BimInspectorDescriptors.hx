...
class BimInspectorDescriptors {
	public static function forElement(element:Element):Array<PropertyDescriptor> {
		...
		result.push(nameDescriptor(element));
		for (property in element.properties())
			result.push(elementProperty(element, property));
		return result;
	}

	public static function forDefinition(definition:Definition):Array<PropertyDescriptor> {
		var result:Array<PropertyDescriptor> = [];
		...
		for (input in definition.inputs())
			result.push(inputDescriptor(definition, input));
		for (property in definition.properties())
			result.push(definitionProperty(definition, property));
		return result;
	}

	private static function inputDescriptor(definition:Definition, input:DefinitionInput):PropertyDescriptor {
		...
		return new PropertyDescriptor("definition:" + definition.id.value + ":input:" + input.name, input.name, PropertyType.Float,
			function(_) return PropertyValue.Float(UnitConversion.fromCanonical(definition.input(input.name).defaultValue,
				...
				case PropertyValue.Int(next): definition.setDefault(input.name, next, input.unit);
				default: throw "Definition input requires a number";
			}, options);
	}

	private static function nameDescriptor(element:Element):PropertyDescriptor {
		var options = new PropertyDescriptorOptions();
		...
		return new PropertyDescriptor("element:" + element.id.value + ":name", "Name", PropertyType.Text,
			function(_) return PropertyValue.Text(element.name), function(_, value) switch (value) {
				case PropertyValue.Text(next): element.rename(next);
				default: throw "Element name requires text";
			}, options);
	}

	private static function elementProperty(element:Element, property:TypedProperty):PropertyDescriptor {
		return propertyDescriptor("element:" + element.id.value, property,
			function(next) element.setProperty(next));
	}

	private static function definitionProperty(definition:Definition, property:TypedProperty):PropertyDescriptor {
		return propertyDescriptor("definition:" + definition.id.value, property,
			function(next) definition.setProperty(next));
	}
