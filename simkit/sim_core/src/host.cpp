...
public:
    ...
    nksim_result submit_body_states(const nksim_body_state *states, std::uint32_t count,
                                    Command::Kind kind = Command::Kind::BodyStates) {
        if (count != 0 && !states)
            return NKSIM_ERROR_INVALID_ARGUMENT;
        Command command;
        command.kind = kind;
        ...
        for (auto &command : pending) {
            ...
            if (command.kind == Command::Kind::Forces) {
                ...
                for (const auto &state : command.body_states) {
                    result = drive ? world->drive_body(state.body, state)
                                   : world->set_body_state(state.body, state);
                    if (result != NKSIM_OK)
                        break;
                }
            }
...
nksim_result NKSIM_HOST_CALL nksim_host_submit_body_drives(
    nksim_host host, const nksim_body_state *states, uint32_t count) {
    const auto value = nksim::resolve_host(host);
    return value ? value->submit_body_states(states, count, nksim::Command::Kind::BodyDrives)
