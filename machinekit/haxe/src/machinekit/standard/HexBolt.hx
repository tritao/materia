...
/** ISO 4017 hex bolt. Threads are semantic (diameter, pitch, length), not modelled.
...
class HexBolt extends MachineComponent {
	...
	static function rows():Array<HexBoltSpec>
		return [
			{size: "M3", diameter: 3, pitch: 0.5, acrossFlats: 5.5, headHeight: 2, threadLength: 18,
				tapDrill: 2.5, clearanceFine: 3.2, clearanceMedium: 3.4, clearanceCoarse: 3.6},
			{size: "M4", diameter: 4, pitch: 0.7, acrossFlats: 7, headHeight: 2.8, threadLength: 20,
				tapDrill: 3.3, clearanceFine: 4.3, clearanceMedium: 4.5, clearanceCoarse: 4.8},
			{size: "M5", diameter: 5, pitch: 0.8, acrossFlats: 8, headHeight: 3.5, threadLength: 22,
				tapDrill: 4.2, clearanceFine: 5.3, clearanceMedium: 5.5, clearanceCoarse: 5.8},
			{size: "M6", diameter: 6, pitch: 1.0, acrossFlats: 10, headHeight: 4, threadLength: 24,
				tapDrill: 5.0, clearanceFine: 6.4, clearanceMedium: 6.6, clearanceCoarse: 7},
			{size: "M8", diameter: 8, pitch: 1.25, acrossFlats: 13, headHeight: 5.3, threadLength: 28,
				tapDrill: 6.8, clearanceFine: 8.4, clearanceMedium: 9, clearanceCoarse: 10},
			{size: "M10", diameter: 10, pitch: 1.5, acrossFlats: 16, headHeight: 6.4, threadLength: 32,
				tapDrill: 8.5, clearanceFine: 10.5, clearanceMedium: 11, clearanceCoarse: 12},
			{size: "M12", diameter: 12, pitch: 1.75, acrossFlats: 18, headHeight: 7.5, threadLength: 36,
	...
	public static function catalog():Catalog<HexBoltSpec> {
		if (table == null)
			table = new Catalog("hex bolt size", spec -> spec.size, rows());
		return table;
	}
	...
	/** Counterbored through-hole tool, sized with a 0.5 mm diametral and axial clearance
	 * around the hex head. The bolt's `head` sits at z=-headHeight-0.5.
	 */
	public function counterboreHole(depth:Float, fit:ClearanceFit = Medium):Part {
		...
		return Solids.union([
			...
			Solids.cylinder(acrossCorners / 2 + 0.5, -seatDepth, 0),
		]);
	}
