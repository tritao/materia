...
void cpp_selection_state() {
    ...
    const nkscene::NodeId first{1};
    const nkscene::NodeId second{2};
    ...
    const auto live = scene->reserve_node_id();
    const auto removed = scene->reserve_node_id();
...
void c_selection_state() {
    ...
    const nkscene_node_id first = {11};
    const nkscene_node_id second = {22};
    ...
    assert(has_hover == 1);
    assert(hovered.node.value == first.value);

    nkscene_scene scene = 0;
    assert(nkscene_scene_create(&scene) == NKS_OK);
    nkscene_transaction transaction = 0;
    assert(nkscene_transaction_begin(scene, &transaction) == NKS_OK);
    nkscene_node_id live = {};
    nkscene_node_id removed = {};
    assert(nkscene_tx_create_node(transaction, &live) == NKS_OK);
    assert(nkscene_tx_create_node(transaction, &removed) == NKS_OK);
    ...
    assert(nkscene_transaction_begin(scene, &transaction) == NKS_OK);
    assert(nkscene_tx_destroy_node(transaction, removed) == NKS_OK);
    assert(nkscene_transaction_commit(transaction) == NKS_OK);
    assert(nkscene_scene_snapshot(scene, &snapshot) == NKS_OK);
    assert(nkscene_interaction_synchronize(interaction, snapshot) == NKS_OK);
...
void selection_presentation_50k() {
    ...
    for (const auto node : nodes) {
        ...
        configure.add_material(node, base_material);
    }
    ...
    SceneInteraction interaction;
    interaction.select(nodes[12345], SelectionMode::Replace);
    auto selected_view = view;
    for (const auto node : interaction.selected())
        selected_view.set_selection_material_override(node, highlight_material);
    selected_view.set_selection_material_override(nodes[12345], hover_material);
    selected_view.set_selection_material_override(nodes[12345], highlight_material);
