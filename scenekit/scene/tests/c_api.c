...
int main(void) {
    ...
    nkscene_node_id group = {0};
    nkscene_node_id first = {0};
    nkscene_node_id second = {0};
    assert(nkscene_tx_create_node(transaction, &group) == NKS_OK);
    assert(nkscene_tx_create_node(transaction, &first) == NKS_OK);
    assert(nkscene_tx_create_node(transaction, &second) == NKS_OK);
    ...
    nkscene_snapshot_node info = {0};
    info.struct_size = sizeof(info);
    int found_group = 0;
    int found_first = 0;
    int found_second = 0;
    for (uint64_t index = 0; index < count; ++index) {
        assert(nkscene_snapshot_get_node(snapshot, index, &info) == NKS_OK);
        if (info.node.value == group.value) {
            found_group = 1;
            assert(info.parent.value == 0);
            assert(info.visible != 0);
        } else if (info.node.value == first.value) {
            found_first = 1;
        ...
        } else if (info.node.value == second.value) {
            found_second = 1;
            assert(info.parent.value == group.value);
    ...
    assert(nkscene_snapshot_get_node(snapshot, count, &info) ==
           NKS_ERROR_INVALID_ARGUMENT);

    nkscene_snapshot_node_page page = {0};
    page.struct_size = sizeof(page);
    assert(nkscene_snapshot_get_node_page(snapshot, 0, &page) == NKS_OK);
    assert(page.start_index == 0);
    assert(page.count == count);
    assert(page.nodes[0].node.value == group.value);
    assert(page.nodes[1].parent.value == group.value);
    assert(nkscene_snapshot_get_node_page(snapshot, count, &page) == NKS_OK);
    assert(page.count == 0);

    uint64_t source_count = 0;
    assert(nkscene_snapshot_get_source_node_count(
               snapshot, first_source, &source_count) == NKS_OK);
    assert(source_count == 1);
    nkscene_node_id source_node = {0};
    assert(nkscene_snapshot_get_source_node(
               snapshot, first_source, 0, &source_node) == NKS_OK);
    assert(source_node.value == first.value);
    assert(nkscene_snapshot_get_source_node(
               snapshot, first_source, source_count, &source_node) ==
           NKS_ERROR_INVALID_ARGUMENT);

    uint64_t child_count = 0;
    assert(nkscene_snapshot_get_child_node_count(snapshot, group, &child_count) == NKS_OK);
    assert(child_count == 2);
    assert(nkscene_snapshot_get_child_node(snapshot, group, 0, &source_node) ==
           NKS_OK);
    assert(source_node.value == first.value);
    assert(nkscene_snapshot_get_child_node(snapshot, group, child_count, &source_node) ==
           NKS_ERROR_INVALID_ARGUMENT);

    uint64_t resource_count = 0;
    assert(nkscene_snapshot_get_geometry_node_count(snapshot, geometry, &resource_count) ==
           NKS_OK);
    assert(resource_count == 2);
    assert(nkscene_snapshot_get_geometry_node(snapshot, geometry, 1, &source_node) ==
           NKS_OK);
    assert(source_node.value == second.value);
    assert(nkscene_snapshot_get_material_node_count(snapshot, material, &resource_count) ==
           NKS_OK);
    assert(resource_count == 2);
    assert(nkscene_snapshot_get_material_node(snapshot, material, 0, &source_node) ==
           NKS_OK);
    assert(source_node.value == first.value);
