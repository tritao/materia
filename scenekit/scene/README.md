...
`nkscene_node_id` identifies one instantiated node in the scene
hierarchy. A source entity may therefore have many nodes, each with its
...
Snapshots preserve the entity/node distinction. A renderer or
selection system can map several visual nodes back to one source entity
