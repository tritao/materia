...
all-edge fillet and chamfer operations are also headless core operations; the
edge finishing API currently targets every edge of a solid and keeps selected
edge lists for a later ABI addition. More operations should be added only
after this boundary is tested from a language-neutral caller.
...
returns aggregate remap counts through the Haxeon document layer.
