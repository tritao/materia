...
/** One target decoded from an RKC3 command frame. */
...
/** Complete atomic target batch decoded from an RKC3 command frame. */
...
/**
 * Fixed-capacity stream decoder for the device-to-host-independent RKC3
 * command format. It has no POSIX or heap dependency and can be ported into a
 * device loop. The handler receives a complete command synchronously only
 * after its envelope, CRC, sequence, and all targets validate.
 */
...
public:
    ...
    /**
     * Feed any number of bytes from the serial stream. Returns the number of
     * complete commands delivered to handler. A null handler consumes no
     * command and does not refresh the watchdog.
     */
...
private:
    static constexpr size_t max_payload_bytes = 20 + 16 * RK_MAX_SERIAL_JOINTS;
    static constexpr size_t max_frame_bytes = 8 + max_payload_bytes + 4;

    void process_buffer(uint64_t received_at_ns, CommandHandler handler, void *context,
