...
static std::vector<std::uint8_t> frame(std::uint8_t type, const std::vector<std::uint8_t> &body) {
    std::vector<std::uint8_t> bytes(device::header_size + body.size() + device::crc_size);
    std::size_t written = 0;
    CHECK(device::encode_frame(type, body, bytes, written));
    CHECK(written == bytes.size());
    return bytes;
}

static void refresh_crc(std::vector<std::uint8_t> &bytes) {
    const auto crc = device::crc32(std::span<const std::uint8_t>(bytes.data(), bytes.size() - 4));
...
struct Collector {
    ...
    std::vector<device::Command> commands;
    bool accept = true;
    ...
    static bool command(void *context, const device::Command &value) {
        auto &self = *static_cast<Collector *>(context);
...
static std::size_t feed(device::CommandDecoder &decoder, const std::vector<std::uint8_t> &bytes,
...
int main(int argc, char **argv) {
    ...
    for (std::size_t split = 0; split <= stream.size(); ++split) {
        device::CommandDecoder decoder(2, fingerprint);
        Collector collector;
    ...
    CHECK(device::max_command_payload == 532 && device::max_state_payload == 796 && device::max_frame_size == 808);
    for (auto baud : rates) {
        const auto command_us = (10ULL * (device::header_size + device::max_command_payload + device::crc_size) * 1000000 + baud - 1) / baud;
        const auto state_us = (10ULL * device::max_frame_size * 1000000 + baud - 1) / baud;
        CHECK(device::response_deadline_us(baud) + 1 >= command_us + state_us + 30000);
        CHECK(device::response_deadline_us(baud) <= command_us + state_us + 30000);
    }
    CHECK(device::response_deadline_us(9600) == 0);
    CHECK(device::response_deadline_us(115200) == 147362);
}
