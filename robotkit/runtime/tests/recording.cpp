...
int main(){
  ...
  assert(rk_recording_writer_create(path.c_str(),8,&w)==RK_OK);
  const char data[]="{\"version\":1,\"ordinal\":\"18446744073709551614\",\"type\":\"sensor\"}";
  assert(rk_recording_writer_enqueue(w,RK_RECORDING_SENSOR,1,UINT64_MAX-1,reinterpret_cast<const uint8_t*>(data),sizeof(data)-1)==RK_OK);
  ...
  assert(rk_recording_reader_next(r,&m,output,&size)==RK_OK);assert(m.ordinal==UINT64_MAX-1);assert(m.kind==RK_RECORDING_SENSOR);assert(size==sizeof(data)-1);assert(std::memcmp(output,data,size)==0);
  ...
  assert(rk_recording_writer_create(incomplete.c_str(),8,&w)==RK_OK);rk_recording_writer_destroy(w);
