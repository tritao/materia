...
std::vector<uint8_t> frame(const std::vector<uint8_t> &payload) {
    std::vector<uint8_t> bytes{'R', 'K', 'C', '3'};
...
std::vector<uint8_t> command(uint32_t kind, uint64_t sequence,
...
int main() {
    ...
    Collector collector;
    const auto mixed = command(RK_COMMAND_JOINT_TARGETS, 1, {
    ...
    for (const uint8_t byte : mixed)
        accepted += decoder.feed(&byte, 1, 100, Collector::accept, &collector);
    ...
    auto damaged = command(RK_COMMAND_NONE, 2);
    damaged.back() ^= 0x40;
    auto heartbeat = command(RK_COMMAND_NONE, 2);
    ...
    assert(feed(decoder, command(RK_COMMAND_NONE, 2), 400, collector) == 0);
    assert(feed(decoder, command(RK_COMMAND_JOINT_TARGETS, 3, {
    ...
    assert(feed(decoder, command(RK_COMMAND_STOP, 3, {{0, RK_TARGET_POSITION, 0.0}}),
                400, collector) == 0);
    assert(feed(decoder, command(RK_COMMAND_NONE, 3, {}, 1), 400, collector) == 0);
    ...
    append_u32(oversized, static_cast<uint32_t>(20 + 16 * RK_MAX_SERIAL_JOINTS + 1));
    const auto after_oversized = command(RK_COMMAND_EMERGENCY_STOP, 4);
    ...
    assert(feed(invalid_model, command(RK_COMMAND_NONE, 1), 1, collector) == 0);

    // A missing command handler must not count traffic as a valid heartbeat.
    robotkit::SerialCommandDecoder no_handler(2);
    const auto unhandled = command(RK_COMMAND_NONE, 1);
    assert(no_handler.feed(unhandled.data(), unhandled.size(), 100, nullptr, nullptr) == 0);
